// frontend/src/components/InspectionsPanel.jsx
import React, { useRef }  from "react";
import { uploadField } from "../api/uploadField";

function InspectionsPanel({ onInspectionAdd, inspections }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      onInspectionAdd(result); // callback to update map
    } catch (err) {
  ...
  return (
    <div className="w-72 bg-white border-r px-4 py-6 shadow-inner transition-all duration-300">
      <h2 className="text-lg font-semibold mb-4 capitalize">Inspecciones</h2>
        <div className="text-gray-600 text-sm">
          {inspections.length === 0 ? (<p className="text-gray-500">Aun no hay inspecciones cargadas.</p>) : (
          <ul className="space-y-4">
            {inspections.map((field, index) => {

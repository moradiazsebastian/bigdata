// frontend/src/components/FieldsPanel.jsx
import React, { useRef }  from "react";
import { uploadField } from "../api/uploadField";

function FieldsPanel({ onGeoJsonAdd, fields }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      onGeoJsonAdd(result); // callback to update map
    } catch (err) {
  ...
  return (
    <div className="w-72 bg-white border-r px-4 py-6 shadow-inner transition-all duration-300">
      <h2 className="text-lg font-semibold mb-4 capitalize">Campos</h2>
        <div className="text-gray-600 text-sm">
          {fields.length === 0 ? (<p className="text-gray-500">Aun no hay campos cargados.</p>) : (
